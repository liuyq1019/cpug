package com.zxelec.cpug.vl.controller;
...
import com.zxelec.cpug.vl.entity.rest.DahuaSubscribeReq;
import com.zxelec.cpug.vl.entity.rest.DahuaSubscribeRsp;
...
import com.zxelec.cpug.vl.util.DateUtils;
...
public class DahuaSubscribeController {
	...
	public DahuaSubscribeRsp batchSubscribe(@RequestBody DahuaSubscribeReq dahuaSubscribeReq) {
		...
		// 设备信息采集订阅
		List<Subscribe> deviceSubscribe = subscribe.stream().filter(c -> "2".equals(c.getSubscribeCategory()))
				.collect(Collectors.toList());
		
		// 过车记录采集订阅
		List<Subscribe> carSubscribe = subscribe.stream().filter(c -> "3".equals(c.getSubscribeCategory()))
				.collect(Collectors.toList());

		// 卡口信息采集订阅
		List<Subscribe> tollgetSubscribe = subscribe.stream().filter(c -> "5".equals(c.getSubscribeCategory()))
				.collect(Collectors.toList());
		
		// 车道订阅采集订阅
		List<Subscribe> laneSubscribe = subscribe.stream().filter(c -> "6".equals(c.getSubscribeCategory()))
				.collect(Collectors.toList());
	...
	@RequestMapping(value = "/VIID/Triggers/Subscribes/{id}", method = RequestMethod.DELETE)
	public DahuaSubscribeRsp delSubscribe(@PathVariable("id") String id, String upSubscribeId) {
		String rspid = customServerProperties.getCpugDomainId() + DateUtils.data2String();
		dahuaCarpassPushService.delSubscribe(id);
		return new DahuaSubscribeRsp("0", "200", "删除成功", rspid, "");
	}
	...
	// * @return
	// */
	// @RequestMapping(value = "/VIID/Triggers/Subscribes",method =
	// RequestMethod.PUT)
	// public DahuaSubscribeRsp batchUpdateTollgateSubscribe(@RequestBody
	// DahuaSubscribeReq dahuaSubscribeReq) {
	...
	// * @return
	// */
	// @RequestMapping(value = "/VIID/Triggers/Subscribes/{ID}",method =
	// RequestMethod.PUT)
	// public DahuaSubscribeRsp cancelSubscribe(@PathVariable("ID") String
	// id,@RequestBody Subscribe subscribe) {
